...
interface Props {
  ...
  error?: string | null;
  /**
   * Picker mode: rows hand the handle to onPick instead of applying it. Used
   * by settings sheets for downstream model application. The effort segment
   * and pending flow are skipped; the caller closes the sheet after picking.
   */
  onPick?: (model: string) => void;
  /** Fires after the sheet closes (pick, back, drag) — the caller uses this
   *  to re-present the settings sheet it dismissed before presenting us. */
  onSheetDismiss?: () => void;
}

export const ModelSheet = forwardRef<BottomSheetModal, Props>(function ModelSheet(
  { models, currentModel, currentEffort, onSelect, error, onPick, onSheetDismiss },
  ref,
) {
  ...
  // Model tap: picker mode hands back the handle. Otherwise, if the model has
  // effort tiers the sheet stays open so the tier can be picked (pending);
  // otherwise it applies immediately and closes.
  ...
  return (
    ...
    >
      ...
      />
      {!onPick && currentModelEfforts.length > 0 ? (
